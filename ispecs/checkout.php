...
<div id="product_box">
   ...
   // $ip=getRealIpAddr();
    $ema=$_SESSION['email'];
   // echo"<script>alert('.$ema.')</script>";
     // echo"<script>alert('.$ema.')</script>";
    $get_customer="select * from signup where email = '$ema'";
    $run_customer=mysqli_query($conn,$get_customer);
$row=mysqli_num_rows($run_customer);
 //echo"<script>alert('.$row.')</script>";
 //echo"<script>alert('.$get_customer.')</script>";
 $rows = mysqli_fetch_assoc($run_customer);
 //$w=$rows['id'];
 //echo"<script>alert('.$w.')</script>";
    //$signup=mysqli_fetch_array($run_customer);
    $customer_id=$rows['id'];
   // echo"<script>alert('.$.')</script>";    //Signup k table se id la kr customer_id mein store krwa rha h
   // echo $customer_id;
...
<?php
 if (!isset($_SESSION['email'])) 
    ...
    else
    {
        ...
        /*
        echo "<div style='border:3px solid #1abc9c;font-weight:bold; font-size:14px; width:600px;padding:30px;margin-left:380px;'>";
        echo "Thankyou...You have submitted your order. Your Order will be deliver within 2-3 working days after your verfication through Call/Email at "; 
        echo $_SESSION['email'];
        echo "</div>";
        */
        ...
        echo "<img src='img/online.png' height='299px' width='299'>";
        echo "<h4><br><br>OR<br><br></h4>";
        echo "<a href = 'order.php?cust_id= $customer_id;'><img src='img/offline.jpg' height='275px' width='275'></a>";
        echo "</html>";
